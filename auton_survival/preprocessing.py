...
class Imputer:
    ...
    def __init__(
        self, cat_feat_strat="ignore", num_feat_strat="mean", remaining="drop"
    ):
        ...
        **kwargs
        ...
        if self._cat_feats:
            if self.cat_feat_strat == "replace":
                self._cat_base_imputer = SimpleImputer(
                    strategy="constant", fill_value=fill_value
                ).fit(df[cat_feats])
            elif self.cat_feat_strat == "mode":
                self._cat_base_imputer = SimpleImputer(
                    strategy="most_frequent", fill_value=fill_value
                ).fit(df[cat_feats])

        ####### NUM VARIABLES
        if self._num_feats:
            if self.num_feat_strat == "mean":
                self._num_base_imputer = SimpleImputer(strategy="mean").fit(
                    df[num_feats]
                )
            elif self.num_feat_strat == "median":
                self._num_base_imputer = SimpleImputer(strategy="median").fit(
                    df[num_feats]
                )
            elif self.num_feat_strat == "knn":
                self._num_base_imputer = KNNImputer(
    ...
    def transform(self, data):
        ...
        if self.cat_feat_strat != "ignore":
            if len(self._cat_feats):
                df[self._cat_feats] = self._cat_base_imputer.transform(
                    df[self._cat_feats]
                )
...
class Preprocessor:
    ...
    def transform(self, data):
        ...
        if self.one_hot:
            data_transformed[self._cat_feats] = data_transformed[
                self._cat_feats
            ].astype("category")
            data_transformed = pd.get_dummies(
                data_transformed, dummy_na=False, drop_first=True
            )
    ...
    def fit_transform(
        ...
        **kwargs
        ...
        imputer_output = self.imputer.fit_transform(
            data,
            cat_feats=cat_feats,
            num_feats=num_feats,
            fill_value=fill_value,
