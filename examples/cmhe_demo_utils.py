...
def plot_synthetic_data(outcomes, features, interventions):
    ...
    for cmap in color_maps:
        color_maps[cmap].set_gamma(0.4)

    # Data
    X1, X2, X3, X4 = (
        features.X1.to_numpy(),
        features.X2.to_numpy(),
        features.X3.to_numpy(),
        features.X4.to_numpy(),
    )
    ...
    ax1.text(
        ...
        color="C0",
        fontsize=fs,
        bbox=dict(lw=2, boxstyle="round", ec="C0", fc=(0.95, 0.95, 0.95)),
    )
    ax1.text(
        1,
        1.75,
        ...
        color="C2",
        fontsize=fs,
        bbox=dict(lw=2, boxstyle="round", ec="C2", fc=(0.95, 0.95, 0.95)),
    )
    ax1.text(
        1,
        -1.75,
        ...
        color="C3",
        fontsize=fs,
        bbox=dict(lw=2, boxstyle="round", ec="C3", fc=(0.95, 0.95, 0.95)),
    )
...
def factual_evaluate(train_data, test_data, horizons, predictions):
    ...
    for i, horizon in enumerate(horizons):
        score = concordance_index_ipcw(
            y_train, y_test, 1 - predictions[:, i], tau=horizon
        )
...
def find_max_treatment_effect_phenotype(g, zeta_probs, factual_outcomes):
    ...
    """
    mean_differential_survival = np.zeros(
        zeta_probs.shape[1]
    )  # Area under treatment phenotype group
...
def find_mean_differential_survival(outcomes, interventions):
    ...
    unique_times = (
        treated_km.survival_function_.index.values.tolist()
        + control_km.survival_function_.index.values.tolist()
    )
    ...
    mean_differential_survival = np.trapz(
        y=(treated_km.values - control_km.values), x=unique_times
    )
