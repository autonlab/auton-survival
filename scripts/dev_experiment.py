...
def experiment(representation='NN', k=3, temp=0.25, dropout=0.5, dataset=1, random_seed=0, censoring=0.0, r_early=125):
    ...
    model.fit(x_tr, t_tr, e_tr, batch_size=512, learning_rate=1e-3,
            iters=200, val_data=train_data, early_stop=False)
    model.fit(x_tr, t_tr, e_tr, batch_size=512, learning_rate=1e-4,
            iters=50, val_data=train_data, early_stop=False)
...
run_all_experiments(param_grid, 'results_bl_1.csv')
